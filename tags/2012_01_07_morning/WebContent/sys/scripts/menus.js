...
function cm_mout(name,cl){
	...
	if(!(this.useframes&&l==0&&this.aobj1)){ this.isover=0; this.aobj1=0; this.tim=setTimeout(this.name+".hideSubs(1,0,0,0,1)",this.wait)} 
}
/************
Onmouseover
*************/
...
function cm_showSubs(name,l,num,cl){
	...
	if(!bw.ns4&&this.checkselect){ //CHECKING FOR SELECT BOXES
		if (this.sel.length == 0) //need to check this condition if menu is hide and user did not change the page
			this.hcode = ""
